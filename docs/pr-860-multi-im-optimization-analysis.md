...
| 2026-08-21 | Superseded the preceding direct-`receiverUid` interpretation: `chat message send --user` now accepts only a contact staffId/userId, resolves it through the current logged-in identity, and dispatches text/media/file messages solely with `receiverOpenDingTalkId`; explicit open IDs remain on `--open-dingtalk-id`, and robotUid is rejected as a recipient identifier. | The live `send_personal_message` MCP schema exposes `receiverOpenDingTalkId` and no `receiverUid`; the previously tested `6871365508` is the sending robot's own robotUid rather than a receiver, so forwarding it bypassed the supported contact identity contract. |
