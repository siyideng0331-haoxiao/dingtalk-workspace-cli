...
`dws deap manage detail` 保持单一命令，通过 `--type` 选择要读取的配置版本：

```text
dws deap manage detail --assistant-id <assistantId> --type draft
dws deap manage detail --assistant-id <assistantId> --type published
```
