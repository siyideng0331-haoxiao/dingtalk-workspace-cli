...
| `DWS_PROFILE` | Bind the current process and inherited child `dws` processes to a profile selector. Prefer the stable `corpId:userId` returned by `dws profile list`. Precedence: explicit `--profile` > `DWS_PROFILE` > the active config directory's `currentProfile`; the environment override does not persist a profile switch. / 将当前进程及继承环境的 `dws` 子进程绑定到指定 profile；推荐使用 `dws profile list` 返回的稳定 `corpId:userId`。优先级：显式 `--profile` > `DWS_PROFILE` > 当前配置目录的 `currentProfile`；环境变量不会持久切换账号。 |
