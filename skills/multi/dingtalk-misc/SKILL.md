...
| DEAP 数字员工 / 数字员工 / 人设 / 群感知 / 临时 DWS 授权码 / 执行状态 / 执行 trace / 响应模式 | DEAP 数字员工管理态（创建/草稿/发布/删除/详情/列表/临时授权码）与执行态（执行状态/trace） | `dws dingtalk-tag` | [deap/deap-index.md](references/deap/deap-index.md) |
...
- DEAP 数字员工的细文档在 [references/deap/](references/deap/)；命中后先读 [deap-index.md](references/deap/deap-index.md)，再按形态加载 [manage.md](references/deap/manage.md)（生命周期）、[run.md](references/deap/run.md)（执行查询）或 [capability.md](references/deap/capability.md)（Skill/MCP 能力资源）。
