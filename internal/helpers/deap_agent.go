...
func newDeapAgentCreateCommand() *cobra.Command {
	return NewLeafCommand(LeafSpec{
		Use:       "create",
		Short:     "创建草稿态数字员工",
		Long:      "创建草稿态 DEAP 数字员工并返回生成的 agentUuid。创建不会自动发布；创建成功后应补齐头像、岗位、响应模式和人设提示词，再调用发布工具。name 和 description 必填，identity 由 MCP 可信注入；员工档案可用独立 flag 或 profile-json 提供，同时提供时独立 flag 覆盖 JSON 同名字段。mainProgramType 仅支持 open_code、local_agent；不传时由 OpenAPI 按 open_code 处理。",
		...
		Flags: []LeafFlag{
			...
			{Name: "main-program-type", Usage: "主程序类型：open_code 或 local_agent；不传时 OpenAPI 按 open_code 处理", Bind: "digitalTagEmployeeProfile.mainProgramType", Trim: true, OmitEmpty: true, Enum: deapAgentMainProgramTypeValues},
		...
		Contract: LeafContract{
			...
			Parameters: []contract.ParamDecl{
				...
				{Name: "main-program-type", Property: "digitalTagEmployeeProfile.mainProgramType", Enum: deapAgentMainProgramTypeValues, Description: "主程序类型；仅支持 open_code、local_agent，不传时 OpenAPI 按 open_code 处理"},
...
func newDeapAgentSaveDraftCommand() *cobra.Command {
	return NewLeafCommand(LeafSpec{
		Use:       "save-draft",
		Short:     "全量覆写数字员工草稿",
		Long:      "全量覆写指定数字员工的基础草稿但不发布。既有基础字段未传会被清空；新增 Skill/MCP 文件参数不传时保持原配置，只有显式提供空数组文件才清空。档案字段可用独立 flag 或 profile-json 提供，同时提供时独立 flag 覆盖 JSON 同名字段。mainProgramType 仅支持 open_code、local_agent；不传时由 OpenAPI 按 open_code 处理。增量修改前必须先查询 detail、保留全部仍需配置的字段（包括 mainProgramType），再整体提交。请先 --dry-run 检查参数，再经用户确认加 --yes。",
...
func deapAgentProfileJSON(raw string) (any, error) {
	...
	if rawProgramType, exists := profile["mainProgramType"]; exists {
		...
		if programType != deapAgentMainProgramTypeOpenCode &&
			programType != deapAgentMainProgramTypeLocalAgent {
			return nil, deapAgentInvalidMainProgramType()
		}
